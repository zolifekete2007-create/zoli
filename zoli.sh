...
# Ha pipe-ból jön (curl ... | bash) ÉS nincs argumentum → udvarias hiba, ne próbáljon olvasni

if [[ -p /dev/stdin && $# -eq 0 ]]; then

  echo -e "${ERR} Így futtatva (curl ... | bash) nem tudok interaktívan kérdezni."

  echo

  echo "Használd így interaktívan:"

  echo "  curl -sL https://raw.githubusercontent.com/zolifekete2007-create/zoli/main/zoli.sh -o zoli.sh"

  echo "  sudo bash zoli.sh"

  echo

  echo "VAGY ha nem akarsz menüt, add meg a választásokat argumentumban pl.:"

  echo "  curl -sL https://raw.githubusercontent.com/zolifekete2007-create/zoli/main/zoli.sh | sudo bash -s 5"

  echo "  curl -sL https://raw.githubusercontent.com/zolifekete2007-create/zoli/main/zoli.sh | sudo bash -s 1 3"
...
# Ha kaptunk argumentumokat (pl. 1 3 4), akkor nem kérdezünk, azokat használjuk
...
if [[ $# -gt 0 ]]; then
  ...
  # Interaktív menü (ekkor NEM pipe-ból jön a stdin, hanem TTY)
